...
void mtxRotated(double M[16], const double q, const double x, const double y, const double z)
{
    ...
    // Column 0;
	T[ 0] = V*x0*x0 + C; //(1.0 - 0)*1*1 + 0 = 1
	T[ 1] = Vxy + Sz; // (1.0 - 0)*1*0 + 1*0 = 0
	T[ 2] = Vxz - Sy; // (1.0 - 0)*1*0 - 1*0 = 0
    T[ 3] = 0.0;
    
    // Column 1;
	T[ 4] = Vxy - Sz; // (1.0 - 0)*1*0 - 1*0 = 0
	T[ 5] = V*y0*y0 + C; // (1.0 - 0)*0*0 + 0 = 0
	T[ 6] = Vyz + Sx; // (1.0 - 0)*0*0 + 1*1 = 1
    T[ 7] = 0.0;
    
    // Column 2;
	T[ 8] = Vxz + Sy;
	T[ 9] = Vyz - Sx; // (1.0 - C)*0*0 - 1*1
	T[10] = V*z0*z0 + C;
    T[11] = 0.0;
